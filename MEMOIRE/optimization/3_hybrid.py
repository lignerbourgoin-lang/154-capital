...
===========================================
Maximise le Sharpe (BL) avec une pénalité Risk Parity :
le portefeuille équilibre rendement ajusté et égalité des risques.

Usage : python 3_hybrid.py
"""
...
def plot_weights(results):
    ...
    ax.set_ylabel("Poids (%)")
    ax.set_title("Hybrid vs RP vs BL — Allocation", fontsize=12, fontweight="bold")
...
def plot_cumulative(returns, results, benchmark):
    ...
    fig.suptitle("Hybrid RP+BL — Performance", fontsize=13, fontweight="bold")
    ...
    for name, r in results.items():
        ...
        lw  = 2.5 if name == "Hybrid" else 1.5
        axes[0].plot(cum.index, cum, label=name, linewidth=lw, color=PALETTE.get(name))
        axes[1].plot(dd.index, dd * 100, label=name, linewidth=lw, color=PALETTE.get(name))
    ...
    plt.tight_layout()
    _save("3_cumulative.png")


def main():
    logger.info("── 3. HYBRID RP + BL ─────────────────────────────────────")
    prices, benchmark = load_universe()
    returns = prices.pct_change(fill_method=None).dropna()

    logger.info("Risk Parity ...")
    rp_result = RiskParityOptimizer(returns, rf=RF).optimize()
    ...
    logger.info("Hybrid RP + BL ...")
    hy = HybridOptimizer(returns, rf=RF)
    hy.add_views(VIEWS)
    hy_result = hy.optimize()
